...
const matchDivision = (line: string, parts: { shell: Range; kernel: Range }[], reg: RegExp) => {
  ...
  for (const part of parts) {
    ...
    completed = completed.slice(0, part.kernel.start) + content + completed.slice(part.kernel.end, line.length);
  }
...
// 装飾なし、変換中の部分に下線が引かれるだけ
export const simpleDecorated = (textAll: string, underline: { start: number; end: number }) => {
  ...
  for (const line of textAll.split("\n")) {
    ...
    let i = 0;
    while (i < line.length) {
      const abs = lastLine + i;
      const inUnderline = abs >= underline.start && abs < underline.end;
      const style = inUnderline ? "text-decoration: underline;" : "text-decoration: none;";
      let j = i;
      while (j < line.length && inUnderline === (lastLine + j >= underline.start && lastLine + j < underline.end)) {
        j++;
      }
      effectLine.push({ part: line.slice(i, j), style });
      i = j;
    }
    effectLines.push(effectLine);
    lastLine += line.length + 1;
  }
  return effectLines;
};
